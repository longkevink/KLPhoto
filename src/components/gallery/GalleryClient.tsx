...
export default function GalleryClient({ photosGrouped }: GalleryClientProps) {
    ...
    const [activeDragPhoto, setActiveDragPhoto] = useState<Photo | null>(null);
    ...
    const handleDragStart = (event: DragStartEvent) => {
        ...
        const photo = event.active.data.current as Photo;
        if (photo) setActiveDragPhoto(photo);
    };
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        if (over && active.data.current) {
            // Check if dropped on a specific slot
            const slotIndex = parseInt(over.id.toString().replace('slot-', ''));
            if (!isNaN(slotIndex)) {
                const photo = active.data.current as Photo;
                const newSlots = [...slots];
                newSlots[slotIndex] = photo;
                setSlots(newSlots);
                setActiveSlotIndex(slotIndex);
            } else if (over.id === 'wall-canvas') {
                // If dropped generally on wall but not a slot, add to active slot or first empty
                const photo = active.data.current as Photo;
                const newSlots = [...slots];
                // Try to fill active slot first if empty, else find first empty
                if (!newSlots[activeSlotIndex]) {
                    newSlots[activeSlotIndex] = photo;
                } else {
                    const nextEmpty = newSlots.findIndex((s, i) => i < currentSlotCount && !s);
                    if (nextEmpty !== -1) {
                        newSlots[nextEmpty] = photo;
                        setActiveSlotIndex(nextEmpty);
                    } else {
                        // Replace visible active
                        newSlots[activeSlotIndex] = photo;
                    }
                }
                setSlots(newSlots);
    ...
    if (!isMounted) {
        return <div className="flex h-[calc(100vh-64px)] pt-[64px] bg-background overflow-hidden font-sans" />;
    }
    ...
    return (
        <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
            ...
            <DragOverlay dropAnimation={null} zIndex={100}>
                {activeDragPhoto ? (
                    <div className={cn(
                        "w-32 rounded-lg overflow-hidden shadow-2xl ring-2 ring-white",
                        activeDragPhoto.orientation === 'landscape' ? "aspect-[3/2]" : "aspect-[2/3]"
                    )}>
                        {/* Simplified Preview */}
                        <img src={activeDragPhoto.src} alt="" className="w-full h-full object-cover" />
                    </div>
                ) : null}
            </DragOverlay>
        </DndContext >
